...
public class PenRepTable extends HttpServlet
{
    ...
    throws IOException, ServletException
    {
        ...
        String dFromOrdDt = request.getParameter("dFrom_Order_Dt");
        String dToOrdDt = request.getParameter("dTo_Order_Dt");
        ...
        if(!( dFromOrdDt==null || dFromOrdDt.equalsIgnoreCase("null") || dFromOrdDt.equalsIgnoreCase("")) )
          vWhereClause += " AND   Order_Dt >= '"+ dFromOrdDt+ "' ";
        if(!( dToOrdDt==null || dToOrdDt.equalsIgnoreCase("null") || dToOrdDt.equalsIgnoreCase("")) )
          vWhereClause += " AND   Order_Dt <= '"+ dToOrdDt+ "' ";
        vWhereClause += " AND ord.fk_client_id=cln.client_id ORDER BY Order_ID ";

       ConfigData cdata = new ConfigData();
       vFormType = cdata.GetConfigValue( "TR_ORDER", nLangID, "WD_FORM_TABLE1", "Pending Order Report / T" );
       Page page = new Page();
       Head head = new Head();
       ...
       Form form = new Form("/JOrder/servlets/PenRepTable", "POST", "_parent",null,null);
       String vCols = " ord.Order_ID, ord.Order_Dt, cln.client_name, ord.ExpDel_Dt ";
       String vTitles = " Order ID, Order Date, Client name, Expected Delivery Date ";
       form.add( new FormHidden("nFk_ItemGroup_ID", nItmGrpID,null)); 
       form.add( new FormHidden("nFk_ItemClass_ID", nItmClsID,null)); 
       form.add( new FormHidden("vTemplName", vTemplName,null));
       form.add( new FormHidden("nPrint", "",null)); 
       form.add(new NL(1));
       form.add(PenRepForm.createPenRepTable(vCols, vWhereClause, vTitles));
       form.add(new NL(2));
       body.add(form);
       page.add(head);
       page.add(body);
    ...
    throws IOException, ServletException
    {
	 ...
	 for( int i=0; i<rOrdNr.size(); i++ )
	 {
       ...
       try
       {
         conn =  db.GetDBConnection();
         stmt = conn.createStatement();
         query = " Select * From T_OrderDtls ort, T_Item itm "+
                 " Where Fk_Order_ID=" + rOrdID.elementAt(i).toString() +
                 " AND ort.Fk_Item_ID = itm.Item_ID ";
         if(!( nItmClsID==null || nItmClsID.equalsIgnoreCase("null") || nItmClsID.equalsIgnoreCase("")) )
           query += " AND   itm.Fk_ItemClass_ID >= '"+ nItmClsID + "' ";
         if(!( nItmGrpID==null || nItmGrpID.equalsIgnoreCase("null") || nItmGrpID.equalsIgnoreCase("")) )
           query += " AND   itm.Fk_ItemGroup_ID <= '"+ nItmGrpID+ "' ";
         query += "ORDER BY Pos_Nr";
